...
const AdminDashboard = ({ products, refreshProducts }) => {
  ...
  // --- UPDATED: Added sizes to initial state ---
  const [formData, setFormData] = useState({ name: '', price: '', image: '', sizes: [] });
  ...
  const openModal = (product = null) => {
    if (product) {
      ...
      setFormData({ name: '', price: '', image: '', sizes: [] });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSizeChange = (size) => {
    const updatedSizes = formData.sizes.includes(size)
      ? formData.sizes.filter((s) => s !== size) // Remove if already there
      : [...formData.sizes, size]; // Add if not there
    ...
    <div className="min-h-screen bg-gray-50 p-8">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="p-4 text-xs uppercase tracking-wider font-semibold text-gray-600">Sizes</th>
                ...
                <tr key={product.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="p-4">
                    <div className="flex gap-1 flex-wrap">
                      {product.sizes?.map(s => (
                        <span key={s} className="px-2 py-0.5 bg-gray-100 text-[10px] font-bold rounded border border-gray-200">{s}</span>
                      ))}
                    </div>
                  </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white w-full max-w-md p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {/* --- NEW: Sizes Checkboxes UI --- */}
              <div>
                <label className="block text-xs uppercase tracking-widest font-bold mb-3 text-gray-500">Available Sizes</label>
                <div className="flex flex-wrap gap-4">
                  {availableSizes.map((size) => (
                    <label key={size} className="flex items-center gap-2 cursor-pointer group">
                      ...
                      />
                      <span className="text-sm font-medium text-gray-700 group-hover:text-black">{size}</span>
                    </label>
                  ))}
                </div>
              </div>
